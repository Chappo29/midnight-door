...
/**
 * Может ли герой шагнуть в клетку: мешают только стены и границы комнаты. Мебель, диван и постройки
 * герой проходит насквозь — поэтому ставить их можно плотно, не боясь запереть себя.
 */
export function heroPassable(r: Room, x: number, y: number): boolean {
  return inRoom(r, x, y);
}
